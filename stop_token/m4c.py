...
class M4C(BaseModel):
    ...
    def _build_obj_encoding(self):
        ...
        self.obj_feat_layer_norm = LayerNorm(self.mmt_config.hidden_size)
        self.obj_bbox_layer_norm = LayerNorm(self.mmt_config.hidden_size)
    ...
    def _build_ocr_encoding(self):
        ...
        self.ocr_feat_layer_norm = LayerNorm(self.mmt_config.hidden_size)
        self.ocr_bbox_layer_norm = LayerNorm(self.mmt_config.hidden_size)
    ...
    def _build_output(self):
        ...
        self.stop_classifier = ClassifierLayer(
            self.config.classifier.type,
            in_dim=self.mmt_config.hidden_size,
            out_dim=1,
            **self.config.classifier.params,
    ...
    def forward(self, sample_list):
        ...
        results["stop_scores"] = fwd_results["stop_scores"]
        return results
    ...
    def _forward_output(self, sample_list, fwd_results):
        ...
        stop_scores = torch.squeeze(self.stop_classifier(mmt_dec_output))
        fwd_results["scores"] = scores
        fwd_results["stop_scores"] = stop_scores
    ...
    def _forward_mmt_and_output(self, sample_list, fwd_results):
        if self.training:
            ...
            for _ in range(dec_step_num):
                ...
                # calculate stop token probability and find indices where
                # the probability is greater than 0.5
                stop_prob = torch.sigmoid(fwd_results["stop_scores"])
                stop_inds = torch.nonzero((stop_prob >= 0.5), as_tuple=False)
                EOS_inds = torch.ones_like(stop_inds)[:, 0].unsqueeze(1)*self.answer_processor.EOS_IDX
                stop_inds = torch.cat((stop_inds, EOS_inds), 1)
                
                # replace score with inf if index is in stop_inds
                fwd_results["scores"][stop_inds[:, 0], stop_inds[:, 1], stop_inds[:, 2]] = math.inf
...
class PrevPredEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.ans_layer_norm = LayerNorm(hidden_size, eps=ln_eps)
        self.ocr_layer_norm = LayerNorm(hidden_size, eps=ln_eps)
        self.emb_layer_norm = LayerNorm(hidden_size, eps=ln_eps)
