...
@registry.register_loss("m4c_decoding_bce_with_mask_stop")
class M4CDecodingBCEWithMaskLossStop(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.0])
    ...
    def forward(self, sample_list, model_output):
        scores = model_output["scores"]
        stop_scores = model_output["stop_scores"]
        targets = sample_list["targets"]

        stop_targets = torch.zeros([list(stop_scores.size())[0], list(
            stop_scores.size())[1]], dtype=torch.float64).to(device='cuda')
        for i in range(list(stop_targets.size())[0]):
            for j in range(list(stop_targets.size())[1]):
                if 2 in torch.nonzero(targets[i, j, :], as_tuple=False).view(-1).cpu().tolist():
                    stop_targets[i, j:] = 1
        ...
        stop_loss = F.binary_cross_entropy_with_logits(
            stop_scores, stop_targets, reduction="none", pos_weight=torch.tensor(20.0))
        losses = F.binary_cross_entropy_with_logits(scores, targets, reduction="none")
        losses *= loss_mask.unsqueeze(-1)
        stop_loss *= loss_mask

        count = torch.max(torch.sum(loss_mask), self.one.to(losses.device))
        loss = (torch.sum(losses) + torch.sum(stop_loss)) / count
        return loss
